...
export function ChangeCredsForm({ onDone }: ChangeCredsFormProps) {
  ...
  const handleSave = () => {
    ...
    updateAdminCreds(newUser.trim(), newPass)
  ...
  return (
    <div className="flex flex-col gap-2 pt-3 border-t border-slate-100">
      ...
      <div className="flex gap-2">
        <button onClick={onDone} className="flex-1 text-xs py-1.5 border border-slate-200 rounded-md text-slate-500 hover:bg-slate-50">
          Cancel
        </button>
        <button
          onClick={handleSave}
          className="flex-1 text-xs py-1.5 rounded-md text-white font-medium"
          style={{ backgroundColor: '#178351' }}
        >
          Save credentials
        </button>
